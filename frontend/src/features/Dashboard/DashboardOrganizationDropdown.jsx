...
// Import Assets
import '../../assets/css/features/Dashboard/DashboardOrganizationDropdown.css';
...
function DashboardOrganizationDropdown({ selectedOrg, onSelect }) {
    const [selectedOrgs, setSelectedOrgs] = useState([]);

    useEffect(() => {
        setSelectedOrgs(selectedOrg ? orgOptions.find(option => option.value.toString() === selectedOrg.toString()) : null);
    }, [selectedOrg]);
    ...
    return (
        <div className="mb-3" title="หน่วยงานที่ดูแลครุภัณฑ์">
            <Select className="select-org-dropdown" value={selectedOrgs} onChange={handleOrgChange} options={orgOptions} placeholder="- เลือกหน่วยงาน -" noOptionsMessage={() => 'ไม่พบตัวเลือก'} isClearable />
        </div>
    );
}

export default DashboardOrganizationDropdown;